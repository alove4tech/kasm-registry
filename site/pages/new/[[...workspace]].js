...
export default function New({ workspace }) {
  ...
  return (
    <div className="">
      ...
      <div className='flex flex-col lg:flex-row w-full my-20 max-w-6xl text-sm rounded-xl overflow-hidden mx-auto border border-slate-800 shadow-2xl'>
        <div className='w-full lg:w-1/2 p-16 bg-slate-900 text-slate-100'>
          ...
          <div className='flex flex-col'>
            ...
            <input type="file" name="icon" onChange={handleChange} className='mb-2 p-2 rounded-lg bg-slate-950 border border-solid border-slate-700 text-slate-200 file:bg-slate-800 file:text-slate-100 file:border-0 file:mr-4 file:px-3 file:py-2' />
            ...
            <input ref={friendly_name} name="friendly_name" onChange={handleChange} className='mb-2 p-2 rounded-lg bg-slate-950 border border-solid border-slate-700 text-slate-100' />
            ...
            <input ref={description} name="description" onChange={handleChange} className='mb-2 p-2 rounded-lg bg-slate-950 border border-solid border-slate-700 text-slate-100' />
            <p className='mb-6 opacity-70'>A short description about the workspace</p>

            <label className='mb-2 font-medium'>Docker Image</label>
            <input ref={name} name="name" onChange={handleChange} className='mb-2 p-2 rounded-lg bg-slate-950 border border-solid border-slate-700 text-slate-100' />
            <p className='mb-6 opacity-70'>The docker image to use, i.e. <code className='text-xs p-1 px-2 rounded bg-slate-800 text-slate-100'>kasmweb/filezilla:develop</code></p>
        ...
        <div className='w-full lg:w-1/2 p-16 bg-slate-950 text-slate-100'>
          <Workspace workspace={combined} icon={icon} inlineImage={inlineImage} />
          <pre className='my-8 overflow-y-auto text-xs text-slate-200'>{JSON.stringify(displayWorkspace(), null, 2)}</pre>
          <button onClick={downloadZip} className='p-4 relative z-10 px-5 bg-sky-600 border border-sky-500 hover:bg-sky-500 transition m-2 rounded items-center text-white flex cursor-pointer'>Download</button>
        </div>
      </div>
    </div>
  )
...
function Workspace({ workspace, icon, inlineImage }) {
  ...
  const editButton = () => {
    return <div className="text-xs w-full p-4 py-1 rounded-lg bg-slate-700 text-slate-100 flex justify-center items-center">Edit</div>
  }
  const official = () => {
    return
  }
  ...
  return (
    <div className={"rounded-xl group w-full shadow-2xl max-w-xs relative overflow-hidden h-[100px] border border-solid flex flex-col justify-between bg-slate-800 border-slate-700 text-slate-100"}>
      <div className={"absolute top-0 left-0 right-0 h-[200px] transition-all" + (showDescription ? ' -translate-y-1/2' : '')}>
        <div onClick={() => setShowDescription(true)} className={"h-[100px] p-4 relative overflow-hidden cursor-pointer"}>
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-slate-950/50 h-8 text-[10px] flex items-center justify-center">
        ...
        <div className="h-[100px] text-xs relative p-2 pl-4 flex">
          <button className="absolute right-2 top-2 bg-slate-700 text-slate-100 rounded-full flex justify-center items-center h-6 w-6" onClick={() => setShowDescription(false)}>
